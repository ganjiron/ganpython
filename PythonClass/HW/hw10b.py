...
def f4(n):
    ...
    a = list(map(lambda y: range(sum(range(y + 1)) - y + 1, sum(range(y + 1)) + 1), range(1, n + 1)))
    print(list(a[-1])[-1])
    b = list(map(lambda y: range(sum(range(y + 1)) - y + 1 + 2 * n - y, sum(range(y + 1)) - y + 1 + 2 * n - y),
                 range(n + 1, 2 * n)))
    for i in a:
        print(*list(map(lambda x: x, i)), sep=' ')
    for i in b:
...
def f10(m1, m2):
    return list(map(lambda x: (
        list(map(lambda y: sum(list(map(lambda z: m1[x][z] * m2[z][y], range(len(m1[0]))))), range(len(m2[0]))))),
                    range(len(m1))))


# 12 return
def f12(rows, cols):
    return list(map(lambda y: list(map(
        lambda x: (1 if ((x - 1) in range(cols)) else 0) + (1 if ((x + 1) in range(cols)) else 0) + (
            1 if ((y - 1) in range(rows)) else 0) + (1 if ((y + 1) in range(rows)) else 0), range(cols))), range(rows)))
